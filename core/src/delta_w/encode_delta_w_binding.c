...
#include "geozl/ctids.h"          // GEOZL_CTID_DELTA_W
...
const ZL_TypedEncoderDesc delta_w_encoder_desc = {
    .gd          = GEOZL_NUM1TO1_GRAPH(GEOZL_CTID_DELTA_W),
...
ZL_Report EI_geozl_delta_w(ZL_Encoder* eictx, const ZL_Input* in)
{
    ...
    delta_w_encode(ZL_Output_ptr(out), ZL_Input_ptr(in), width, nbElts, eltWidth);
