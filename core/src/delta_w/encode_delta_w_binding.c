...
ZL_Report EI_geozl_delta_w(ZL_Encoder* eictx, const ZL_Input* in)
{
    ...
    // the C graph builder attaches the tile row width as a local int param
    ZL_IntParam wp     = ZL_Encoder_getLocalIntParam(eictx, DELTA_W_PARAM_WIDTH);
    const uint32_t wid = (wp.paramId == DELTA_W_PARAM_WIDTH)
    ...
    delta_w_encode(ZL_Output_ptr(out), ZL_Input_ptr(in), wid, nbElts, eltWidth);
