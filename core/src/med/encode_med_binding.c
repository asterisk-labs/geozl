...
ZL_Report EI_geozl_med(ZL_Encoder* eictx, const ZL_Input* in)
{
    ...
    ZL_IntParam wp     = ZL_Encoder_getLocalIntParam(eictx, MED_PARAM_WIDTH);
    const uint32_t wid = (wp.paramId == MED_PARAM_WIDTH)
    ...
    med_encode(ZL_Output_ptr(out), ZL_Input_ptr(in), wid, nbElts, eltWidth);
