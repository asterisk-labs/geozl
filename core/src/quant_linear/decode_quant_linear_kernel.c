...
// x = q * scale. Integer output multiplies in 64 bit and clamps to the type
// range so the top of the range does not wrap. Float output multiplies in
// double and casts to the original float width. The index stream is read at the
// same width as the original integer type, or at a signed integer of the float
// width.
...
void quant_linear_decode(void *dst, const void *src, double scale, int dtype,
  ...
  case QL_U64:
    ...
    break;
  case QL_I8:
    QL_DEC_I(int8_t, INT8_MIN, INT8_MAX);
    break;
  case QL_I16:
    QL_DEC_I(int16_t, INT16_MIN, INT16_MAX);
    break;
  case QL_I32:
    QL_DEC_I(int32_t, INT32_MIN, INT32_MAX);
    break;
  case QL_I64:
    QL_DEC_I(int64_t, INT64_MIN, INT64_MAX);
    break;
  case QL_F16: {
