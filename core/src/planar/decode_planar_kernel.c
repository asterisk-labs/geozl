...
// later row folds the row above into the residual, c[i] = res[i] + N[i] - NW[i],
// and prefix sums it in the same pass, so the W chain is resolved without a
// separate fold. Row major, @width samples per row. @dst may alias @src.
...
static void frow8(uint8_t* d, const uint8_t* res, const uint8_t* above, size_t n)
{
    ...
    for (; i + 32 <= n; i += 32) {
        ...
        __m128i loTot = _mm_broadcastb_epi8(_mm_srli_si128(_mm256_castsi256_si128(c), 15));
        c = _mm256_add_epi8(c, _mm256_inserti128_si256(_mm256_setzero_si256(), loTot, 1));
        c = _mm256_add_epi8(c, carry);
        _mm256_storeu_si256((__m256i*)(d + i), c);
        carry = _mm256_broadcastb_epi8(_mm_srli_si128(_mm256_extracti128_si256(c, 1), 15));
    }
#elif PLANAR_SSE2
...
static void frow16(uint16_t* d, const uint16_t* res, const uint16_t* above, size_t n)
{
    ...
    for (; i + 16 <= n; i += 16) {
        ...
        __m128i loTot = _mm_broadcastw_epi16(_mm_srli_si128(_mm256_castsi256_si128(c), 14));
        c = _mm256_add_epi16(c, _mm256_inserti128_si256(_mm256_setzero_si256(), loTot, 1));
        c = _mm256_add_epi16(c, carry);
        _mm256_storeu_si256((__m256i*)(d + i), c);
        carry = _mm256_broadcastw_epi16(_mm_srli_si128(_mm256_extracti128_si256(c, 1), 14));
    }
#elif PLANAR_SSE2
    ...
    for (; i + 8 <= n; i += 8) {
        uint16x8_t c = vsubq_u16(vaddq_u16(vld1q_u16(res + i), vld1q_u16(above + i)),
                                 vld1q_u16(above + i - 1));
...
static void frow32(uint32_t* d, const uint32_t* res, const uint32_t* above, size_t n)
{
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        c = _mm256_add_epi32(c, _mm256_inserti128_si256(_mm256_setzero_si256(), loTot, 1));
        c = _mm256_add_epi32(c, carry);
        _mm256_storeu_si256((__m256i*)(d + i), c);
        carry = _mm256_broadcastd_epi32(_mm_shuffle_epi32(_mm256_extracti128_si256(c, 1), 0xFF));
    }
#elif PLANAR_SSE2
    ...
    for (; i + 4 <= n; i += 4) {
        uint32x4_t c = vsubq_u32(vaddq_u32(vld1q_u32(res + i), vld1q_u32(above + i)),
                                 vld1q_u32(above + i - 1));
...
static void frow64(uint64_t* d, const uint64_t* res, const uint64_t* above, size_t n)
{
        ...
        size_t eltWidth)
{
    ...
    switch (eltWidth) {
        case 1: PLANAR_DEC(uint8_t, scan8, frow8); break;
        case 2: PLANAR_DEC(uint16_t, scan16, frow16); break;
        case 4: PLANAR_DEC(uint32_t, scan32, frow32); break;
        case 8: PLANAR_DEC(uint64_t, scan64, frow64); break;
        default: break;
    }
}
