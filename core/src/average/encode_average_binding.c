#include "encode_average_binding.h"
...
ZL_Report EI_geozl_average(ZL_Encoder* eictx, const ZL_Input* in)
{
    ...
    ZL_IntParam wp     = ZL_Encoder_getLocalIntParam(eictx, AVERAGE_PARAM_WIDTH);
    const uint32_t wid = (wp.paramId == AVERAGE_PARAM_WIDTH)
    ...
    // output is a fresh buffer, no aliasing with the input
    average_encode(ZL_Output_ptr(out), ZL_Input_ptr(in), wid, nbElts, eltWidth);
