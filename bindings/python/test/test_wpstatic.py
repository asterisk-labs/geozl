...
def _make_tile(shape, dtype, pattern):
    ...
    rng = np.random.default_rng(zlib.crc32(f"{shape}-{dt.str}".encode()))
    if np.issubdtype(dt, np.integer):
        info = np.iinfo(dt)
...
def _anisotropic(h, w, seed):
    """A field stretched along one diagonal, so the plane predictor is not optimal
    and a trained kernel can beat it."""
...
# round trips bit exact on every width and shape, whatever kernel it fits, exactly
# like the planar default it falls back to
...
def test_round_trip(dtype, shape, pattern):
    ...
    out = _roundtrip(geozl.lossless.WpStatic(shape[1]), arr)
    assert np.array_equal(out, arr.reshape(-1))


# on a directional tile the trained kernel beats the plane predictor by more than
# the header bytes it costs, so the C trainer really does fit
@pytest.mark.parametrize("seed", [1, 2, 3])
def test_training_beats_planar(seed):
    arr = _anisotropic(256, 256, seed)
    wp = len(_frame(geozl.lossless.WpStatic(256), arr))
    planar = len(_frame(geozl.lossless.Planar(256), arr))
    assert wp < planar


# on plane friendly data the trainer falls back, so wp_static never costs more than
# its header over planar, it does not settle on a worse kernel
def test_fallback_not_worse():
    arr = _make_tile((64, 64), np.uint16, "gradient")
    wp = len(_frame(geozl.lossless.WpStatic(64), arr))
    planar = len(_frame(geozl.lossless.Planar(64), arr))
    assert wp <= planar + 16


# the wp_static header layout, matched to the C binding
_HEADER = "<IB4h"
...
def _trained_kernel(arr):
    ...
    frame = _frame(geozl.lossless.WpStatic(arr.shape[1]), arr)
    d = zl.DCtx()
...
# the kernel the C trainer picks must reach an entropy at least as good as the numpy
# oracle on the same tile. the coefficients may differ, ridge versus SVD, what has
# to match is the entropy they achieve, measured the same way for both
@pytest.mark.parametrize("seed", [1, 2, 3])
